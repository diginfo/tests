...
dwap.page.trace_onChange = function(nv,ov){          
...
// also fires on click.
dwap.page.notrace_onChange = function(nv,ov){
  var me = $(this);
  var dg = $('#receiptlines');
...
// trace ID selection.
dwap.page.trace = function(me,rec){
  //console.log('dwap.page.trace:',rec);
  var tdg = $('#tracedg');
  ...
  columns: [[
    ...
    {field:'RECEIVE_QTY',title:'Qty Rcv',width:60,fixed:true,align:'right',
      ...
      editor:{type:'numberspinner',options:{
        ...
        id:'rcv_qty',
        readonly: false,
        onChange: dwap.page.notrace_onChange
      }
    }},
...
dwap.page.saverows = function(id){
  ...
  if(dwap.bhave.SAVEALL_LINES != 'y') dels.reverse().map(function(e){dg.datagrid('deleteRow',e)});  var rowdat = jsonString(rows);
...
dwap.page.isloading = function(){ return $('#receiptlines').form('options').loading}
...
$(document).ready(function(){
  ...
  $('#receiptlines')
    .datagrid('rowEditor',dwap.page.opts)
  ...
  */
      ...
      {field:'QTY',title:'Qty Rcv',width:70, fixed:true,
        editor:{
          ...
          options:{
            ...
            value:0,
            readonly: false,
            onChange: dwap.page.trace_onChange
          }
        }
      },
  ...
  $('form#receipthead').on('loadDone',function(jq,data){
    ...
    if(mr.res._next) dwap.page.saverows(mr.res._next);
  })
