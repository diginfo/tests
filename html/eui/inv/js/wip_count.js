...
dwap.page.opts = {
  ...
  dwap.page.jobs=function(reload){
  ...
  dwap.page.saverows = function(id,jobclass){
  ...
  $(document).ready(function() {
    ...
    $('#but_add').on('done',function(){
       // dwap.page.jobs({_func:'get',_sqlid:'inv^wip_count_jobs',_dgrid:'y'});
       ...
       dwap.page.endis(endis);
        ...
        if(mr.res._next) {
          var jobclass=$('#WO_CLASS').combobox('getValue');
            dwap.page.saverows(mr.res._next,jobclass);
           /* $('#lines').datagrid('load',{
                    _func:'get',
                    _sqlid:'inv^wip_count_lines',
                    _dgrid:'y',
          ...
          if (res.error) return ;
            confirm(function(yn){
              if (yn){
                ...
                ajaxget('/',{_sqlid:'inv^wip_count',_func:'upd', WIP_COUNT_ID:id,END_DATE:new Date(),END_COUNT_USER:dwap.udata.userid},function(res){
                    $('#endcount').linkbutton('disable');
                    $('#pwd').textbox('clear');
                });
              }
